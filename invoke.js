...
async function invoke() {
	...
	try {
		...
		const peer = fabric_client.newPeer('grpc://192.168.99.100:7051');
		console.log('Created client side object to represent the peer');
		// -- orderer instance to reprsent the channel's orderer
		const orderer = fabric_client.newOrderer('grpc://192.168.99.100:7050')
		...
		const proposal_request = {
			targets: [peer],
			chaincodeId: 'fabcar',
			fcn: 'changeCertifInstitution',
			args: ['PERSON0', 'RYERSON'],
			chainId: 'mychannel',
			txId: tx_id
		};
